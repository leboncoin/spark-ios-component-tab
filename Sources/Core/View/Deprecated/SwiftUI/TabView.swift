...
@available(*, deprecated, message: "Use SparkTab instead")
public struct TabView: View {
    private let intent: TabIntent
    private var viewModel: TabViewModelDeprecated<TabItemContent>
    @ObservedObject private var containerViewModel = TabContainerViewModelDeprecated()
    ...
    public init(theme: any Theme,
        ...
        let viewModel = TabViewModelDeprecated(
            theme: theme,
            content: content,
            tabSize: tabSize
        )
    ...
    public var body: some View {
        ...
        if self.containerViewModel.apportionsSegmentWidthsByContent {
            TabApportionsSizeViewDeprecated(viewModel: viewModel, intent: self.intent, selectedIndex: self.$selectedIndex)
        } else {
            TabEqualSizeViewDeprecated(viewModel: viewModel, intent: self.intent, selectedIndex: self.$selectedIndex)
        }
    }
